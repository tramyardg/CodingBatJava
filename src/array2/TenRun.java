    ...
    public int[] tenRun(int[] nums) {
        ...
        for (int i = 0; i < nums.length - 1; i++) {
            ...
            if (nums[i] % 10 == 0) {
                ...
                // your current index is multiple by 10
                // then if the next index is not multiple
                // by 10, assigned the next index equal to
                // the current number (multiple by 10)

                // example: [2, 10, 3, 4, 20, 5, 1]
                // here the first mutiple by 10 is at index 1
                // the next number is 3 at index 2
                // this is NOT mutiple by 10 so while it is not
                // multiple by 10 then make the next number 
